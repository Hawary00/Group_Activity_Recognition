...
from model import Person_Activity_Temporal_Classifer, Group_Activity_Temporal_Classifer
...
# Loss Function
loss_fun = torch.nn.CrossEntropyLoss(ignore_index=-100, label_smoothing=0.1)
# Optimizer
...
# Load person model
saved_person_model_path = "/mnt/New Volume/Deep Learning. DR Mostafa/Group_Activity_Recognition/models/baseline7/outputs/trained_model/a/model_20250723_141648_epoch5" #take it from baseline7 ### make sure to select right path
person_model = Person_Activity_Temporal_Classifer(num_classes=9,
...
# Use in group model
model = Group_Activity_Temporal_Classifer(person_model, hidden_size=512,
                                        num_layers=2,
                                        num_classes=8)
model.to(device)
...
# Lambda function: halve LR every 5 epochs
lr_lambda = lambda epoch: 0.5 ** (epoch // 5)
...
log_dir = f"/mnt/New Volume/Deep Learning. DR Mostafa/Group_Activity_Recognition/models/baseline8/outputs/tensorbord/baseline8__{timestamp}"
...
for epoch in range(EPOCHS):
    ...
    # scheduler.step()
    scheduler.step(avg_vloss)  # <- adjust learning rate based on validation loss
    ...
    if avg_vloss < best_vloss:
        ...
        model_save_dir = "/mnt/New Volume/Deep Learning. DR Mostafa/Group_Activity_Recognition/models/baseline8/outputs/trained_model/"
